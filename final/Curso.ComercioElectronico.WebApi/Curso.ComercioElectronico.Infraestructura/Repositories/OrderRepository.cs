...
namespace Curso.ComercioElectronico.Infraestructura.Repositories
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task<OrderItem> AddProductAsync(OrderItem orderItem)
        {
            ...
            else
            {
                ...
                orderProductExist = await cntext.OrderItems.Where(x => x.OrderId == orderFind.Id 
                                                                    && x.ProductId == product.Id).SingleOrDefaultAsync();
        ...
        {
            OrderItem orderProductFind = await cntext.OrderItems.Where(x => x.ProductId == orderItem.ProductId && x .OrderId == orderId).SingleOrDefaultAsync();
        ...
        public async Task<Product> ValidateQuantityStock(OrderItem orderProduct, char operation = 'C')
        {
            //Validar que la cantidad sea mayor a 0
            if (orderProduct.QuantityProduct <= 0)
                throw new ArgumentException("La cantidad seleccionada debe ser mayor a 0");

            //Consultar y validar si existe el producto seleccionado
            Product product = await cntext.Products.FindAsync(orderProduct.ProductId);
            if (product == null)
                throw new ArgumentException($"No existe el producto con Id: {orderProduct.ProductId}");
            if (operation != 'U')
            {
                //Validar que la cantidad seleccionada sea menor al stock del producto, y que el stock es 0
                if (product.Stock < orderProduct.QuantityProduct || product.Stock == 0)
                    throw new ArgumentException($"No suficiente stock del producto, cantidad disponible: {product.Stock}, cantidad seleccionada: {orderProduct.QuantityProduct}");
            }
        ...
        public async Task<bool> RemoveProductAsync(Guid orderId, Guid productId)
        {
            ...
            OrderItem orderProduct = await cntext.OrderItems.Where(x => x.OrderId == orderId && x.ProductId == productId).SingleOrDefaultAsync();
        ...
        {
            return cntext.Orders.Where(x => x.Id == orderId).AsQueryable();
        }
        ...
        public async Task<IQueryable<Order>> CancelAsync(Guid orderId)
        {
            ...
            {
                List<OrderItem> orderItems = await cntext.OrderItems.Where(x => x.OrderId == orderId).ToListAsync();
            ...
            {
                productSelected = await cntext.Products.Where(x => x.Id == orderItem.ProductId).SingleAsync();
                productSelected.Stock += orderItem.QuantityProduct;
                await cntext.SaveChangesAsync();
    ...
    public async Task<Order> UpdateOrderAsync(Guid orderId, decimal subtotal, decimal totalPrice)
        {
            Order orders = await cntext.Orders.FindAsync(orderId);
            if (orders == null)
                throw new ArgumentException($"No existe la orden con id: {orderId}");

            orders.Subtotal = subtotal;
            orders.TotalPrice = totalPrice;
            cntext.Entry(orders).State = EntityState.Modified;
            await cntext.SaveChangesAsync();
            return orders;
